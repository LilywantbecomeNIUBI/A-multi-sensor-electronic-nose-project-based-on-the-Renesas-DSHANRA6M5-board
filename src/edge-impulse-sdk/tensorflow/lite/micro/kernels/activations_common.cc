...
void ReluQuantized(const ReluOpData& data, const RuntimeShape& input_shape,
  ...
  for (int i = 0; i < flat_size; ++i) {
    ...
    clamped = std::max(data.params.quantized_activation_min, clamped); // @suppress("函数无法被解析")
    clamped = std::min(data.params.quantized_activation_max, clamped); // @suppress("函数无法被解析")
...
void CalculateReluOpData(const TfLiteTensor* input, TfLiteTensor* output,
                         ReluOpData* data) {
  float act_min = 0.0;
  float act_max = std::numeric_limits<float>::infinity(); // @suppress("函数无法被解析") // @suppress("Symbol is not resolved")
  ...
  data->params.quantized_activation_min = std::max( // @suppress("函数无法被解析")
      static_cast<int32_t>(std::numeric_limits<T>::min()), // @suppress("函数无法被解析") // @suppress("Symbol is not resolved")
      output->params.zero_point +
          static_cast<int32_t>(roundf(act_min / output->params.scale)));
  data->params.quantized_activation_max =
      act_max == std::numeric_limits<float>::infinity() // @suppress("函数无法被解析") // @suppress("Symbol is not resolved")
          ? static_cast<int32_t>(std::numeric_limits<T>::max()) // @suppress("函数无法被解析") // @suppress("Symbol is not resolved")
          : std::min(static_cast<int32_t>(std::numeric_limits<T>::max()), // @suppress("函数无法被解析") // @suppress("Symbol is not resolved")
                     output->params.zero_point +
                         static_cast<int32_t>(
